...
export function WorkersPage({ workers, loaded, error }) {
  return <Page title="Workers" description="The machines available to pick up and execute work.">{error && <Failure value={error} />}{!loaded && !error ? <Loading description="Checking live worker status." /> : loaded && (workers.length ? <Card className="overflow-hidden">{workers.map((worker) => <article key={worker.instance_id} className="grid gap-4 border-b border-border p-4 last:border-b-0 sm:grid-cols-[minmax(12rem,1fr)_minmax(12rem,1fr)_10rem] sm:items-center sm:px-5">
    <div className="min-w-0"><div className="flex items-center gap-2"><Server className="size-4 text-muted-foreground" /><h2 className="truncate text-sm font-medium">{worker.name}</h2></div><p className="mt-1 truncate font-mono text-xs text-muted-foreground">{worker.instance_id}</p>{worker.fleet_release && <p className="mt-1 truncate font-mono text-xs text-muted-foreground" title={worker.fleet_release}>Release {worker.fleet_release.slice(0, 12)}</p>}</div>
    <div className="flex flex-wrap gap-1.5">{worker.repositories?.length ? worker.repositories.map((repository) => <Badge key={repository} className="border-border bg-muted font-mono text-muted-foreground">{repository}</Badge>) : <span className="text-xs text-muted-foreground">No repositories</span>}{!worker.accepting_work && <Badge className="border-warning/25 bg-warning/10 text-warning">Draining</Badge>}{worker.release_state === "error" && <Badge className="border-danger/25 bg-danger/10 text-danger">Release error</Badge>}{worker.release_state !== "error" && !worker.release_compatible && <Badge className="border-warning/25 bg-warning/10 text-warning">Release mismatch</Badge>}{worker.release_compatible && !worker.release_current && <Badge className="border-border bg-muted text-muted-foreground">Canary window</Badge>}</div>
